...
const addAddress = async (req, res) => {
  try {
    ...
    if (!addressRegex.test(trimmedAddress)) {
      return res.status(400).json({
        message: "Địa chỉ chỉ được chứa chữ, số, dấu '/' và ','",
      });
    }
    ...
    if (!hasLetter || !hasNumber) {
      return res.status(400).json({
        message: "Địa chỉ phải chứa cả chữ và số",
      });
    }
    ...
    const formattedAddress = {
      ...
      createdAt: formatVNDateTime(newAddress.createdAt),
    };
...
const updateAddress = async (req, res) => {
  try {
    ...
    if (trimmedReceiver !== undefined) {
      ...
      if (trimmedReceiver.length < 2 || trimmedReceiver.length > 100)
        return res.status(400).json({ message: "Tên người nhận phải từ 2 đến 100 ký tự" });
    ...
    if (trimmedAddress !== undefined) {
      ...
      const hasNumber = /[0-9]/.test(trimmedAddress);
      if (!hasLetter || !hasNumber)
        return res.status(400).json({ message: "Địa chỉ giao hàng phải chứa cả chữ và số" });
      if (trimmedAddress.length < 10 || trimmedAddress.length > 255)
        return res.status(400).json({ message: "Địa chỉ giao hàng phải từ 10 đến 255 ký tự" });
...
const deleteAddress = async (req, res) => {
  try {
    ...
    if (address.is_default) {
      return res.status(400).json({ message: "Không được xóa địa chỉ mặc định" });
    }
    ...
    return res.status(200).json({
      ...
      data: { user: formattedUser, deletedAddress },
    });
  } catch (error) {
...
const setDefaultAddress = async (req, res) => {
  try {
    ...
    await model.user_addresses.update({ is_default: false }, { where: { user_id } });
